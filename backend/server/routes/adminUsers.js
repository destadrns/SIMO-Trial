...
import { getAppBaseUrl, sendInviteEmail } from '../utils/email.js';
...
async function createInvite(db, actor, payload, actionType = 'INVITE_USER') {
  ...
  return {
    ...
    delivery: delivery.mode === 'development-response-only' ? { ...delivery, inviteToken: invite.token, inviteUrl, expiresAt } : delivery,
  };
}
...
export function createAdminUsersRouter(db) {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    const rows = await all(db, `${USER_SELECT} ORDER BY u.created_at DESC, u.name`);
  ...
  router.post('/:id/resend-invite', asyncHandler(async (req, res) => {
    ...
    sendData(res, { delivery: delivery.mode === 'development-response-only' ? { ...delivery, inviteToken: invite.token, inviteUrl, expiresAt } : delivery });
  }));
  ...
  router.patch('/:id/role', asyncHandler(async (req, res) => {
    ...
    await run(db, 'UPDATE users SET role_id = ? WHERE id = ?', [nextRoleId, user.id]);
    await writeAuditLog(db, {
      actor: { id: req.user.id, name: req.user.name, roleName: req.user.roleName },
      module: 'User Management',
  ...
  router.patch('/:id/status', asyncHandler(async (req, res) => {
    ...
    await run(db, 'UPDATE users SET account_status = ?, is_active = ?, disabled_at = CASE WHEN ? = ? THEN CURRENT_TIMESTAMP::text ELSE disabled_at END WHERE id = ?', [nextStatus, nextStatus === 'ACTIVE' ? 1 : 0, nextStatus, 'DISABLED', user.id]);
    await writeAuditLog(db, {
      actor: { id: req.user.id, name: req.user.name, roleName: req.user.roleName },
      module: 'User Management',
